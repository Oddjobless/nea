...
class Particle:
    ...
    def calculate_density(self, particle):
        density = 0
        neighbouring_particles = self.vector_field.get_neighbouring_particles(self, particle)
        for neighbour_particle in neighbouring_particles:
            if self.vector_field.get_magnitude(neighbour_particle.position - particle.position):
                pass
